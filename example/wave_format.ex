defmodule Example.WaveFormat do
  ...
  def read_fmt_subChunk({fmt, bytes}) do
  ...
  def read_data_subChunk({fmt, bytes}) do
  ...
  def decode_amplitude("", points) do
    Enum.reduce(points, {[], 0}, fn point, {rev, len} -> {[point | rev], len + 1} end)
  end

  def decode_amplitude(<<point::int16LE(), rest::binary>>, points) do
